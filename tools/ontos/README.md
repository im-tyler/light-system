...
Third independent implementation of the ontos stream spec (normative: ontos repo, `docs/STREAM_SPEC.md`) — Rust sim (ontos), Python verifier (simval), this C++17 std-only re-simulator. Verifies both format versions: v1 life (Phase 0) and v2 gravity (Phase 2: leapfrog + Chebyshev ephemeris windows + momentum ledger, re-simulated bit-exactly with bounded drift reported against an all-fine reference; section 19 Phase 4 collapse/expand: RegionCollapsed totals, monopole gravity, jitter + residual-velocity synthesis). Spike toward the ontos Phase 3 viewer bridge: JoltViewer thin-consumer pattern, zero coupling to the renderer core.
